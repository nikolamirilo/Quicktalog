...
export default function ConsentModal({ isOpen, onConfirm, onCancel }: ConsentModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="text-product-foreground w-[95vw] max-w-md mx-4 p-6 sm:p-8 bg-product-background border border-product-border shadow-product-shadow rounded-2xl">
        <DialogHeader className="space-y-3">
          ...
          <DialogDescription className="text-product-foreground-accent text-base leading-relaxed">
            Please review and accept our terms and conditions to continue with your account creation.
          </DialogDescription>
        </DialogHeader>
