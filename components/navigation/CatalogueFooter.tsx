...
import { FiExternalLink, FiHome, FiMail, FiMapPin, FiPhone, FiPlus, FiShield, FiStar, FiUsers, FiZap } from 'react-icons/fi';
...
const CatalogueFooter: React.FC<CatalogueFooterProps> = ({ 
  ...
  customPartnerBadges = [
    { name: 'Partner 1', logo: '/partners/partner1.svg', description: 'Business Partner', rating: 4.8 },
    { name: 'Partner 2', logo: '/partners/partner2.svg', description: 'Business Partner', rating: 4.9 },
    { name: 'Partner 3', logo: '/partners/partner3.svg', description: 'Business Partner', rating: 4.7 }
  ]
}) => {
  ...
  const partnerBadges = [
    { name: 'Glovo', logo: '/partners/glovo.svg', description: 'Delivery Partner', rating: 4.8, url: 'https://glovo.com' },
    { name: 'Wolt', logo: '/partners/wolt.svg', description: 'Delivery Partner', rating: 4.9, url: 'https://wolt.com' },
    { name: 'Uber Eats', logo: '/partners/uber-eats.svg', description: 'Delivery Partner', rating: 4.7, url: 'https://ubereats.com' }
  ];
  ...
  const PartnerBadge = ({ partner }: { partner: { name: string; logo: string; description: string; rating: number; url?: string } }) => {
    ...
    return (
      ...
      >
        ...
        <div className="flex items-center space-x-1">
          ...
          <span className="text-xs font-medium" style={{ color: 'var(--card-description)' }} aria-label={`Rating: ${partner.rating} out of 5`}>{partner.rating}</span>
        </div>
      </div>
    );
  };
  ...
  return (
    <footer className="border-t mt-auto" role="contentinfo" aria-label={`${type === 'default' ? 'Quicktalog' : customCompanyName} footer`} style={{
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
            ...
            <div className="space-y-6">
              <div className="space-y-4">
                <Link href="/" className="flex flex-col items-start space-y-2 group transition-transform duration-200 hover:scale-105" aria-label={`Go to ${type === 'default' ? 'Quicktalog' : customCompanyName} homepage`}>
                  <Image
                    src={logoPath}
              ...
              {type === 'custom' && (
                <nav className="flex items-center space-x-3" role="navigation" aria-label="Social media links">
            ...
            <div className="space-y-6">
              ...
              <ul className="space-y-4">
                <li>
                  ...
                  >
                    <FiMail className="w-4 h-4 group-hover:scale-110 transition-transform duration-200" aria-hidden="true" />
                ...
                <li>
                  <a 
                    ...
                    style={{ color: 'var(--card-description)' }}
                  >
                    <FiPhone className="w-4 h-4 group-hover:scale-110 transition-transform duration-200" aria-hidden="true" />
                    <span>{type === 'default' ? '+1 (234) 567-890' : customPhone}</span>
                  </a>
                </li>
                <li className="flex items-start space-x-3">
                  <FiMapPin className="w-4 h-4 mt-1 flex-shrink-0" style={{ color: 'var(--card-description)' }} />
                  <span className="text-sm" style={{ color: 'var(--card-description)' }}>
                    {type === 'default' ? 'San Francisco, CA' : `${customAddress}, ${customLocation}`}
                  </span>
                </li>
              </ul>
            </div>
                ...
                <>
                  ...
                  <ul className="space-y-4">
                    {features.map((feature, index) => (
                      <li key={index} className="flex items-start space-x-3 group" style={{ color: 'var(--card-description)' }}>
                ...
                <>
                  ...
                  <ul className="space-y-4">
                    ...
                    <li className="flex items-center space-x-3">
                      <FiHome className="w-4 h-4 flex-shrink-0" style={{ color: 'var(--card-description)' }} />
                      <span className="text-sm" style={{ color: 'var(--card-description)' }}>{customLegalName}</span>
                    </li>
                  </ul>
                </>
              )}
        ...
        <div className="border-t py-6" style={{ borderTop: '1px solid var(--section-border)' }}>
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-sm" style={{ color: 'var(--card-description)' }}>
            <span>© {new Date().getFullYear()} {type === 'default' ? 'Quicktalog' : customLegalName}. All rights reserved.</span>
            <nav className="flex items-center space-x-6" role="navigation" aria-label="Legal links">
              <Link href="/privacy-policy" className="hover:text-primary transition-colors duration-200" aria-label="Privacy Policy">
                Privacy Policy
              </Link>
              <Link href="/terms-and-conditions" className="hover:text-primary transition-colors duration-200" aria-label="Terms of Service">
                Terms of Service
              </Link>
              <Link href="/refund-policy" className="hover:text-primary transition-colors duration-200" aria-label="Refund Policy">
                Refund Policy
              </Link>
            </nav>
              ...
              {type === 'custom' && (
                <div>
                  <form onSubmit={handleNewsletterSubmit} className="flex items-center space-x-2" role="form" aria-label="Newsletter subscription">
                    ...
                    {submitError && (
                      <p className="text-red-500 text-xs absolute -bottom-6 left-0" role="alert" aria-live="polite">
                        {submitError}
                      </p>
                    )}
                  </form>
                  <p id="newsletter-description" className="text-xs mt-2" style={{ color: 'var(--card-description)' }}>
