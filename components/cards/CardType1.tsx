...
const CardType1 = ({
  ...
  return (
    <article className="flex flex-row bg-card-bg text-card-text rounded-[12px] border border-card-border
shadow-lg overflow-hidden max-w-full min-h-[110px] sm:min-h-[150px]" role="article" aria-labelledby={`item-title-${record.name?.replace(/\s+/g, '-').toLowerCase()}`} tabIndex={0}>
