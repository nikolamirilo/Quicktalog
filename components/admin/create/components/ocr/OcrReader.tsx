...
const OcrReader = ({ formData }) => {
  ...
  const handleImageChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.files) {
      ...
      const newImages: ImageData[] = newFiles.map((file, index) => ({
        id: `${Date.now()}-${index}`,
        file,
  ...
  const updateImageData = (imageId: string, updates: Partial<ImageData>) => {
    setImages((prev) => prev.map((img) => (img.id === imageId ? { ...img, ...updates } : img)))
  }

  const processImage = async (imageData: ImageData, index: number) => {
  ...
  const extractTextFromAllImages = async () => {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      const extractedText = await processImage(imageData, i)
                    ...
                    <div className="text-center">
                      ...
                      {imageData.isProcessed && (
                        <div className="flex justify-center">
                          {imageData.confidence !== undefined && imageData.confidence > 0 ? (
                            <span
