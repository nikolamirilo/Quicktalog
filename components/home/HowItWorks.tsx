  ...
  const steps = [
    {
      ...
      description:
        "Upload your existing content or choose a flexible template. Our platform works for any service business.",
    ...
    },
    {
      ...
      description:
        "Easily customize your catalogue—no design or tech skills needed. Add your branding, organize services, and set prices.",
    ...
    },
    {
      ...
      description:
        "Share your catalogue with a link or QR code. Your customers can access it 24/7 on any device.",
          ...
          <motion.div
            ...
            <div className="relative bg-product-background rounded-2xl p-8 shadow-lg hover:shadow-xl border border-product-border transition-all duration-300 h-full">
              ...
              <div className="flex items-center justify-center w-16 h-16 bg-product-primary/10 rounded-xl mb-6">
                <div className="text-product-primary">{step.icon}</div>
              </div>

              {/* Title and description */}
              <h3 className="text-xl font-semibold text-product-foreground mb-4">{step.title}</h3>
              ...
              <ul className="space-y-3">
                {step.details.map((detail, detailIndex) => (
                  <li key={detailIndex} className="flex items-start gap-3">
                    ...
                    <span className="text-product-foreground-accent text-sm">{detail}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            {index < steps.length - 1 && (
              <div className="hidden md:block absolute top-1/2 -right-4 w-8 h-0.5 bg-product-primary/20 transform -translate-y-1/2 z-0"></div>
            )}
          </motion.div>
        ))}
      </div>
