...
const FlipCard: React.FC<FlipCardProps> = ({
    ...
    const MetricCard = ({ isBack = false }: { isBack?: boolean }) => (
        <div className={`${isBack
            ...
            <div className="relative z-10 flex flex-col justify-center h-full">
                <div className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-2 font-lora ${isBack ? 'text-product-background' : 'text-product-foreground'}`}>
                    {isBack ? '✓' : metric}
                </div>
                <div className={`text-sm sm:text-base font-medium font-lora ${isBack ? 'text-product-background/90' : 'text-product-foreground-accent'}`}>
            ...
            <div className={`absolute -bottom-1 -right-1 sm:-bottom-2 sm:-right-2 opacity-10 text-4xl sm:text-5xl md:text-6xl ${isBack ? 'text-product-background' : 'text-product-foreground-accent'}`}>
                {icon}
            </div>
        </div>
    );
    ...
    const ContentCard = ({ isBack = false }: { isBack?: boolean }) => (
        <div className={`bg-product-background ${CARD_STYLES.rounded} ${CARD_STYLES.minHeight} ${CARD_STYLES.padding} ${CARD_STYLES.transition} hover:scale-[var(--product-hover-scale)] hover:shadow-[var(--product-hover-shadow)] border border-product-border ${isBack ? 'hover:bg-product-hover-background' : ''
            } flex flex-col w-full`}
            style={{ boxShadow: 'var(--product-shadow)' }}>
            <div className="flex flex-col h-full">
                <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                    <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center ${isBack
                        ...
                        {isBack ? <FiCheck className="w-4 h-4 sm:w-5 sm:h-5" /> : <FiX className="w-4 h-4 sm:w-5 sm:h-5" />}
                    </div>
    ...
    const problems = [
        {
            ...
            problem: "You want a beautiful, professional-looking catalog, but you're not a designer. Hiring one is expensive, and DIY tools are clunky and time-consuming.",
            solution: "Create a stunning, mobile-first catalog in minutes with our easy-to-use templates. No design or coding skills needed. Your business will look professional and polished.",
        ...
        },
        {
            ...
            problem: "Your menu or service list changes frequently, but updating it is a nightmare. You have to call your designer, wait for them to make the changes, and then pay them for their time.",
            solution: "Update your catalog in real-time, anytime, from any device. Add new services, change prices, and mark items as sold out with a single click. No more waiting, no more extra costs.",
        ...
        },
        {
            ...
            problem: "You spend a fortune on printing and reprinting menus and catalogs. Every time you make a change, you have to throw away the old ones and print new ones. It's wasteful and expensive.",
            solution: "Go green and save money with a digital-first catalog. Share your catalog via a link or QR code, and never worry about printing costs again. Plus, you'll be doing your part for the environment.",
