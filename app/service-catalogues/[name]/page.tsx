...
const page = async ({ params }: { params: Promise<{ name: string }> }) => {
  try {
    ...
    if (!data || data.length === 0) {
      return (
        <div className={`theme-elegant bg-background text-foreground min-h-screen flex flex-col`} role="application" aria-label="Service Catalogue">
          <CatalogueHeader />
          <main className="flex-1 flex flex-col justify-center items-center px-4 py-16 sm:py-24" role="main" aria-label="Service catalogue content">
    ...
    return (
      <div className={ "theme-creative bg-background text-foreground min-h-screen flex flex-col"} role="application" aria-label={`${item.title} Service Catalogue`}>
        ...
        <main className="flex-1 flex flex-col min-h-0" role="main" aria-label="Service catalogue content">
          {/* Hero Section */}
          <section className="flex flex-col justify-start items-center text-center px-4 pt-8 sm:pt-12 md:pt-16 flex-shrink-0" aria-labelledby="catalogue-title">
            <div className="max-w-4xl mx-auto">
              <h1 id="catalogue-title" className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-lora font-semibold text-heading drop-shadow-sm mb-4">
                {item.title}
              </h1>
              {item.subtitle && (
                <p className="text-text text-base sm:text-lg md:text-xl lg:text-2xl px-5 max-w-[900px] font-lora font-normal leading-relaxed" aria-describedby="catalogue-title">
                  {item.subtitle}
                </p>
              )}
            </div>
          ...
          {/* Services Section */}
          {item && (
            <section className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 sm:pt-12 md:pt-16 pb-8 min-h-[60vh]" aria-label="Services and items">
              <ServicesSection
    ...
    return (
      <div className={`theme-elegant bg-background text-foreground min-h-screen flex flex-col`} role="application" aria-label="Service Catalogue Error">
        <CatalogueHeader />
        <main className="flex-1 flex flex-col justify-center items-center px-4 py-16 sm:py-24" role="main" aria-label="Error content">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-2xl" role="alert" aria-live="polite">
